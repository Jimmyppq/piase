                ...
                while True:
                    try:
                        ...
                        transactions[:] = [
                            ...
                            if t['first_action'] == 'NEWTRANS' or t['Last Action'] == 'NEWTRANS'
                            ]
            ...
            for i in range(self.index_block):
                if block_process != (i+1) :
                    ...
                    for incomplete_transaction in transactions:
                        ...
                        cannocial_trx_id = self.uf.find(transaction_id_incomplete)
                ...
                while True:
                    try:
                        ...
                        for transaction in transactions:                            
                            ...
                            m_trx_id = transaction.get('m_transaction_id')
                            ...
                            elif transaction['first_action'] == 'MNEWTRANS' and m_trx_id:
                                self.uf.add_transaction(transaction_id)
                                self.uf.add_transaction(m_trx_id)
                                self.uf.union(transaction_id, m_trx_id)  # Vincular las transacciones
   ...
   # completadas que se han modificado previamente para complementarla con el indice de incompletas si es que existian. 
    ...
    def clear_binary_file(self):
        ...
        if os.path.exists(self.reviewTransactionsFile):
            ...
            self.logger.debug(f"El archivo binario {self.reviewTransactionsFile} no existía, no es necesario eliminarlo.")
        
        if os.path.exists(self.resultFinalFile) :
            try:
                os.remove(self.resultFinalFile)
                self.logger.info(f"Archivo CSV {self.resultFinalFile} eliminado correctamente.")
            except Exception as e:
                self.logger.error(f"Error al intentar eliminar el archivo CSV {self.resultFinalFile}: {e}")
                sys.exit(1)  # Detener la ejecución si no se puede limpiar el archivo binario
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        '''uf = reader.process_batch_incomplete_transactions()
        reader.process_large_binary_file(uf)'''
